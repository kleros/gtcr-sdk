...
export class GTCRFactory {
  ...
  constructor(_ethereum: ethers.providers.ExternalProvider | ethers.providers.JsonRpcFetchFunc , _factoryAddress: string, _deploymentBlock: number = 0, blockTimeMilliseconds?: number) {
  ...
  public async getTCRAddresses() {
    ...
    const addresses = (await Promise.all(
      intervals.map(async interval => this.gtcrFactoryInstance.queryFilter(
